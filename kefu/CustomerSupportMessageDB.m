...
#import <gobelieve/ReverseFile.h>
#import "CustomerConversation.h"

@interface CustomerSupportMessageIterator : IMessageIterator

@end

@implementation CustomerSupportMessageIterator


-(IMessage*)nextMessage {
    if (!self.file) return nil;
    return [CustomerSupportMessageDB readMessage:self.file];
}
...
@implementation CustomerSupportConversationIterator

-(IMessage*)getLastMessage:(NSString*)path {
    CustomerSupportMessageIterator *iter = [[CustomerSupportMessageIterator alloc] initWithPath:path];
    IMessage *msg;
    //返回第一条不是附件的消息
    while (YES) {
        msg = [iter next];
...
@implementation CustomerSupportMessageDB
+(CustomerSupportMessageDB*)instance {
    static CustomerSupportMessageDB *m;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!m) {
            m = [[CustomerSupportMessageDB alloc] init];
        }
    });
    return m;
}
...
-(NSString*)getPeerPath:(int64_t)uid appID:(int64_t)appID {
    NSString *s = self.dbPath;
    return [NSString stringWithFormat:@"%@/c_%lld_%lld", s, appID, uid];
}


-(BOOL)insertMessage:(IMessage*)msg uid:(int64_t)uid appID:(int64_t)appID {
    NSString *path = [self getPeerPath:uid appID:appID];
...
-(id<IMessageIterator>)newMessageIterator:(int64_t)uid appID:(int64_t)appID {
    NSString *path = [self getPeerPath:uid appID:appID];
    return [[CustomerSupportMessageIterator alloc] initWithPath:path];
}

-(id<IMessageIterator>)newMessageIterator:(int64_t)uid appID:(int64_t)appID last:(int)lastMsgID {
    NSString *path = [self getPeerPath:uid appID:appID];
    return [[CustomerSupportMessageIterator alloc] initWithPath:path position:lastMsgID];
}

-(id<ConversationIterator>)newConversationIterator {
    NSString *path = [self getMessagePath];
    return [[CustomerSupportConversationIterator alloc] initWithPath:path];
}
